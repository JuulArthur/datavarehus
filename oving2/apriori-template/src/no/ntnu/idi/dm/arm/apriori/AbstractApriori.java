...
public abstract class AbstractApriori<V> {
	...
	public void generateRulesBase(ItemSet<V> frequentItemSet,
        ...
        //Get support for the left hand side of the expression
        Double leftHandSupport = supportCache.get(frequentItemSet);

        if (support!=null){
            rule.setSupport(support);
            //Confidence = support of the set / support of the left hand side of the set
            rule.setConfidence(support/leftHandSupport);
            rules.add(rule);
        }
